...
// Map Netwealth exchange codes to proper Yahoo Finance symbols.
//   .ND = Nasdaq (US)      -> no suffix
//   .NY = NYSE   (US)      -> no suffix
//   .LN = London           -> .L
//   .CO = Copenhagen       -> .CO  (keep; Yahoo uses .CO)
//   no dot = ASX domestic  -> .AX  (e.g. MTO, LNW, MFG, MGOC)
// Canonical merges: some securities appear under multiple Netwealth codes
// (dual listings or post-restructure tickers) but are ONE position for the user.
// Map every variant code to a single canonical ticker + Yahoo exchange suffix.
const CANONICAL = {
  'LNW':       'LNW.AX',   // Light & Wonder CDI (ASX)
  'LNW.ND':    'LNW.AX',   // Light & Wonder Nasdaq -> fold into ASX line
  'WISE.LN':   'WISE.L',   // Wise Group PLC (London)
  'NWWISE.LN': 'WISE.L',   // Wise PLC post-restructure -> fold into London line
};

function cleanTicker(code) {
  if (!code) return null;
  let c = code.toUpperCase().trim();
  if (CANONICAL[c]) return CANONICAL[c];
  const dot = c.lastIndexOf('.');
  if (dot === -1) {
    // No exchange code in the Netwealth file = ASX-listed domestic security
    return c + '.AX';
  }
  ...
  switch (ex) {
    case 'ND':            // Nasdaq US
    case 'NY': return base;            // NYSE US -> plain US ticker
    case 'LN': return base + '.L';     // London Stock Exchange
    case 'CO': return base + '.CO';    // Copenhagen
    case 'AX':
    case 'ASX': return base + '.AX';   // already ASX
    default: return base;              // unknown -> strip
  }
}
...
function extractTransactions(rows) {
  ...
  rows.forEach(row => {
    ...
    // Skip ALL foreign-exchange cash positions (FXUSD, FXGBP3, FXDKK, FXEUR, FXJPY, …).
    // These are foreign-currency cash balances, not equities — track them in the Cash tab.
    if (/^FX/i.test(code)) return;
    if (!code) return;
